...
const getProduct = async (req, res) => {
    try {
        console.log('Bắt đầu lấy danh sách sản phẩm', { path: req.path, acceptHeader: req.headers.accept });
        ...
        for (let product of products) {
            ...
            const detailsVariants = await DetailsVariant.find({ productId: product._id }).lean();
...
const getProductById = async (req, res) => {
    try {
        console.log('Bắt đầu lấy sản phẩm theo ID', { productId: req.params.productId });
...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        console.log('Tạo tên file ảnh', { originalName: file.originalname, newFilename: filename });
        cb(null, filename);
    },
});
...
const addProduct = async (req, res) => {
    try {
        console.log('Bắt đầu thêm sản phẩm mới', { body: req.body, file: req.file });
        ...
        if (hasVariants) {
            for (const detail of parsedVariantDetails) {
                ...
                if (!Array.isArray(variantDetailsArray) || variantDetailsArray.length === 0) {
            ...
            const variantCombinations = parsedVariantDetails.map(detail =>
                detail.variantDetails.map(v => `${v.variantId}:${v.value}`).sort().join('|')
            );
        ...
        const newProduct = new Product({
            ...
            hasVariants,
            variants: hasVariants ? parsedVariantDetails.map(detail => ({
        ...
        console.log('Sản phẩm đã được thêm thành công', { productId: savedProduct._id, productName: savedProduct.name });
...
const updateProduct = async (req, res) => {
    try {
        console.log('Bắt đầu cập nhật sản phẩm', { productId: req.params.productId, body: req.body });
        ...
        console.log('Sản phẩm đã được cập nhật thành công', { productId, productName: updatedProduct.name });
...
const deleteProduct = async (req, res) => {
    try {
        ...
        console.log('Sản phẩm đã được xóa thành công', { productId, productName: product.name });
