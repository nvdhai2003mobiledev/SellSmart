...
const addProduct = async (req, res) => {
    try {
        ...
        const { name, price, thumbnail, description, category, inventory, status, attributes } = req.body;
...
const addProductVariant = async (req, res) => {
    try {
        ...
        const result = await productService.addProductVariant(productId, variantData);
        return res.status(result.status === 'Error' ? 400 : 200).json(result);
    } catch (error) {
        console.error('Error:', error);
        return res.status(500).json({ 
...
const updateVariantDetail = async (req, res) => {
    try {
        ...
        const result = await productService.updateVariantDetail(detailId, updatedData);
        return res.status(result.status === 'Error' ? 400 : 200).json(result);
    } catch (error) {
        console.error('Error:', error);
        return res.status(500).json({ 
