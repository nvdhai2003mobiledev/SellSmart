...
exports.getAllProviders = async (req, res) => {
    try {
        ...
        res.render("provider", { providers, page: "provider", admin: req.user || null });
...
exports.searchProviderByPhone = async (req, res) => {
    try {
        ...
        if (!phone) {
            return res.status(400).json({ message: "Vui lòng nhập số điện thoại để tìm kiếm!" });
        }

        const providers = await Provider.find({ phoneNumber: { $regex: phone, $options: "i" } });
